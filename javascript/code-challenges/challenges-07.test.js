...
const sortStarWarsCharacters = (starWarsArr) => {
  let finalArray = starWarsArr.sort ((a,b) => a.height - b.height);
  return finalArray;
};
...
const listFoods = (recipe) => {
  ...
  for (let i=0; i < recipe.ingredients.length; i++) {
    firstArray= recipe.ingredients[i].split('');
    secondArray= firstArray.slice(2);
    result.push(secondArray.join(""));
  }
  return result;
};
